<?php

include_once "template.php";

$content = '<div id="centro">
    Cadastro de usuário: 
    <form action="cadastro.php" method="post">
    Nome: <input type="text" name="nome"><br>
    Login: <input type="text" name="login"><br>
    Senha: <input type="password" name="passwd"><br>
    <input type="hidden" value="1" name="status">
    <input type="hidden" value="U" name="tipo">
    <input type="submit" value="Cadastrar">
    </form>
    <!--CAROUSEL-->
    <div class="html_carousel">
        <div id="foo3">
            <!--CONTEÚDO DO HOVER-->
            <div class="slide">
                <img src="img/image_1.jpg" alt="carousel 1" width="620" height="355" />
                <div>
                    <h4>Noticia 1</h4>
                    <p>Aqui vai a preview da noticia 1</p>
                </div>
            </div>
            <div class="slide">
                <img src="img/image_2.jpg" alt="carousel 2" width="620" height="355" />
                <div>
                    <h4>Noticia 2</h4>
                    <p>Aqui vai a preview da noticia 2</p>
                </div>
            </div>
            <div class="slide">
                <img src="img/image_3.jpg" alt="carousel 3" width="620" height="355" />
                <div>
                    <h4>Noticia 3</h4>
                    <p>Aqui vai a preview da noticia 3</p>
                </div>
            </div>
            <div class="slide">
                <img src="img/image_4.jpg" alt="carousel 4" width="620" height="355" />
                   <div>
                    <h4>Noticia 4</h4>
                    <p>Aqui vai a preview da noticia 4</p>
                   </div>
               </div>
            </div>
        <div class="clearfix"></div>
    </div>
    <!--CAMPO DE NOTÍCIAS-->
    <div id="campo_noticias">
        <div class="noticias">
            <a href="#"><img src="img/image_1.jpg" width="150" height="100"></a>
            <a href="#"><h2>Notícia 1</h2></a>
            <p>Aqui vai as informaçoes da noticia 1</p>
            <a href="#" id="readmore">read more...</a>
            <p id="data">07 de Julho de 2013</p>
        </div>
        <div class="noticias">
            <a href="#"><img src="img/image_2.jpg" width="150" height="100"></a>
            <a href="#"><h2>Notícia 2</h2></a>
            <p>Aqui vai as informaçoes da noticia 2</p>
            <a href="#" id="readmore">read more...</a>
            <p id="data">07 de Julho de 2013</p>
        </div>
        <div class="noticias">
            <a href="#"><img src="img/image_3.jpg" width="150" height="100"></a>
            <a href="#"><h2>Notícia 3</h2></a>
            <p>Aqui vai as informaçoes da noticia 3</p>
            <a href="#" id="readmore">read more...</a>
            <p id="data">07 de Julho de 2013</p>
        </div>
        <div class="noticias">
            <a href="#"><img src="img/image_4.jpg" width="150" height="100"></a>
            <a href="#"><h2>Notícia 4</h2></a>
            <p>Aqui vai as informaçoes da noticia 4</p>
            <a href="#" id="readmore">read more...</a>
            <p id="data">07 de Julho de 2013</p>
        </div>
    </div>
</div>';

echo $top;
echo $content;
echo $bottom;

?>